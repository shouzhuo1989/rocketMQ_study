        ...
        if (pair != null) {
            Runnable run = new Runnable() {
                ...
                public void run() {
                    try {
                        /**
                         * 作为客户端  我就是发出请求   得到回应    至于你服务端怎么接收请求  怎么给我回应我不管
                         * netty作为请求接收组件   tomcat和severlet结合的时候  有一个severletContext
                         * 现在我们有一个channelHandlerContext   就是当一个handler拿到一个请求时，通过channelHandlerContext，它可以拿到该请求对应的channel
                         * 也可以拿到channel对应的eventLoop，也可以拿到该eventLoop对应的thread
                         */
