 ...
 * specified.
 * 一种基于机房邻近优先级的分配策略代理。可以指定实际的分配策略。
 *
 * If any consumer is alive in a machine room, the message queue of the broker which is deployed in the same machine
 * should only be allocated to those.
 * 如果一个机房中有存活的consumer，则同一台机器上部署的broker上的message queue应该分配给这些consumer
 * Otherwise, those message queues can be shared along all consumers since there are
 * no alive consumer to monopolize them.
 * 否则，这些消息队列可以由所有消费者共享，因为没有活动消费者垄断它们。
 */
